...
test('a school opens a sourced promotion and its larger corpus remains accessible', async ({ page }, info) => {
  ...
  expect((await graph(page)).nodes.length).toBeLessThanOrEqual(14);
  await expect(page.getByRole('navigation', { name: 'Pages du réseau' })).toContainText('sur 123');
  const first = (await graph(page)).nodes;
  await page.getByRole('button', { name: 'Page suivante du réseau', exact: true }).click();
  await expect(page).toHaveURL(/page=1/);
  await expect.poll(async () => (await graph(page)).nodes).not.toEqual(first);
  expect((await graph(page)).nodes).toContain('Q3052772');
  const pageUrl = page.url();
  await ready(page, pageUrl);
  await expect(page.getByRole('navigation', { name: 'Pages du réseau' })).toContainText('2 / 11');
