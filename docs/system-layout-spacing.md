...
## Intégration avec la nouvelle interface de main

Les commits plus récents de main introduisent une organisation propre à chaque catégorie. La vue « Ensemble » utilise explicitement les positions précalculées avec l'espacement des pôles, tout en conservant l'adaptation au format du canvas et la mémorisation du cadrage. Les vues thématiques conservent leurs ancrages spécifiques. Ce raccordement évite que l'organisation de la nouvelle interface remplace les positions améliorées.

Validation de l'intégration : 28 fichiers et 132 tests unitaires réussis, lint et build de production réussis (TypeScript compris). Le navigateur intégré confirme le rendu de la vue Ensemble, le passage à Formation, le retour à Ensemble et le zoom. L'aperçu du résultat intégré utilise le port 4312.
