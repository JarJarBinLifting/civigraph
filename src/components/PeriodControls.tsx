...
export function PeriodControls({ data, view, onChange, onEvidence, onSelect, onCareer }: {
  ...
  return <section className="period-controls" aria-label="Exploration par période">
    ...
    <p className="period-summary" role="status">{view.temporal === 'same' ? `${matching} lien${matching > 1 ? 's' : ''} sur la période retenue.` : 'Toutes les participations du corpus, quelle que soit leur période.'}{unknown > 0 && ` ${unknown} lien${unknown > 1 ? 's' : ''} sans période exploitable.`}{possible > 0 && ` ${possible} chevauchement${possible > 1 ? 's' : ''} incertain${possible > 1 ? 's' : ''}.`}</p>
