...
export function NetworkInsights({ data, relations, view, onChange, onReveal }: {
  ...
  return <section className="network-insights" aria-label={isCircles ? 'Cercles entre partis' : 'Parcours entre milieux'}>
    <header className="insights-heading"><span className="eyebrow">Lire les réseaux politiques</span><h2>{isCircles ? 'Quels cercles partagent-ils ?' : 'Qui relie plusieurs milieux ?'}</h2>
      <p>{isCircles ? 'Écoles, commissions, associations : retrouvez les espaces fréquentés par des personnes aux appartenances politiques différentes.' : 'Suivez les parcours documentés entre partis, haute administration, cabinets, entreprises, médias, formations et cercles.'}</p>
    </header>
    ...
    <p className="insights-count" role="status">{count} {isCircles ? 'cercles' : 'personnes avec au moins deux milieux'} dans cette lecture</p>
