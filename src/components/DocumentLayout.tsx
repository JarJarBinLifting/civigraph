...
export function DocumentLayout({ children }: { children: React.ReactNode }) {
  return <div className="document-shell">
    <header className="document-header"><Link className="brand" href="/" prefetch={false}><span className="brand-symbol"><Network size={22} /></span>civigraph</Link><nav aria-label="Navigation documentaire"><Link href="/" prefetch={false}>Explorer la carte</Link><a href="/methode">Méthode et couverture</a></nav></header>
