...
import { systemPalette } from '@/lib/graph-theme';
...
export function PoliticalLegend({ politics, entities }: { politics: PoliticalIndex; entities: Entity[] }) {
  ...
  return <div className="political-legend" aria-label="Légende des appartenances politiques">
    <div className="atlas-legend-preview" aria-label="Repères de la carte"><strong>Lire la carte</strong>{([
      ['person', 'Personnes'], ['school', 'Formation'], ['office', 'Fonctions'], ['party', 'Politique'], ['organization', 'Organisations'],
    ] as const).map(([type, label]) => <span key={type}><i className={type === 'person' ? 'person-marker' : 'institution-marker'} style={{ background: type === 'person' ? systemPalette[type] : '#fff', borderColor: systemPalette[type] }} />{label}</span>)}</div>
    <details name="map-options" className="political-legend-details" onKeyDown={event => {
      ...
      <p><strong>Lire la carte.</strong> Un clic ouvre la fiche et met les connexions en évidence. Double-cliquez pour privilégier les noms du voisinage. La disposition ne mesure ni influence ni proximité personnelle.</p>
      <div className="political-legend-types">{Object.entries(typeInfo).filter(([type]) => type !== 'person').map(([type, info]) => <span key={type}><i style={{ background: systemPalette[type as keyof typeof systemPalette] }} />{info.label}</span>)}</div>
      <p>Les contours des institutions indiquent leur type. Les personnes apparaissent en bleu nuit dans la vue d’ensemble ; leurs appartenances politiques se révèlent au zoom, à la sélection ou dans la lecture Individus.</p>
