...
export function GuidedNetworks({ data, view, onChange, onReveal, onOpenView }: { onOpenView: (view: ViewState) => void; data: GraphData; view: ViewState; onChange: (patch: Partial<ViewState>) => void; onReveal: (id: string) => void }) {
  ...
  return <section className="network-insights guided-networks" aria-label="Explorations guidées">
    ...
    {!group ? <>
      ...
      <p className="insights-scope">{groups.filter(g => g.kind === 'cabinet').length} cabinets et {groups.filter(g => g.kind === 'promotion').length} promotions ou programmes. Les sélections sont partielles ; elles ne désignent pas nécessairement la première équipe d’une carrière.</p>
      <div className="guide-catalogue">{shown.map(g => <button className="insight-card" key={g.id} onClick={() => { onChange({ journey: g.id, journeyStep: 0 }); setCopyMessage(''); requestAnimationFrame(() => heading.current?.focus()); }}><span className="eyebrow">{g.kind === 'cabinet' ? 'Cabinet ministériel' : 'Promotion / programme'}</span><strong>{g.label}</strong><span>{g.members.length} {g.members.length === 1 ? 'personne recensée' : 'personnes recensées'}</span><span className="insight-action">Explorer en trois étapes →</span></button>)}</div>
      ...
      <h3>{group.label}</h3><p className="insights-scope">{group.members.length} {group.members.length === 1 ? 'membre documenté' : 'membres documentés'} dans cette sélection.</p><details className="guide-coverage"><summary>Qui figure dans cette sélection ?</summary><p>{group.members[0].relations[0].evidence?.note}</p></details>
