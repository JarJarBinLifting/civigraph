...
export function ComparisonGraph({ left, right, common, selected, onSelect }: { left: Entity; right: Entity; common: CommonConnection[]; selected: string; onSelect: (id: string) => void }) {
  ...
  useEffect(() => {
    ...
    import('cytoscape').then(({ default: cytoscape }) => {
      ...
      const instance = cytoscape({ container: container.current, elements, layout: { name: 'preset', fit: false }, minZoom: .01, maxZoom: 2.5, wheelSensitivity: .2,
        style: [
          ...atlasNodeStyles,
          ...
          { selector: 'edge', style: { label: 'data(label)', color: atlasTheme.ink, 'font-family': 'Arial, sans-serif', 'text-background-color': atlasTheme.paper, 'text-background-opacity': .95, 'line-color': 'data(color)', 'target-arrow-color': 'data(color)', 'target-arrow-shape': 'triangle', 'curve-style': 'bezier', 'overlay-opacity': 0, 'overlay-padding': 5, opacity: .65 } },
      ...
      const labels = () => {
        ...
        instance.batch(() => {
          ...
          for (const edge of instance.edges()) edge.style({ width: (edge.is('.active, .hover') ? 1.8 : .8) / zoom, 'font-size': 12 / zoom, 'text-background-padding': 3 / zoom, 'text-opacity': instance.width() >= 500 && zoom >= .65 ? 1 : 0 });
        });
      };
    ...
    return () => { disposed = true; observer?.disconnect(); cancelAnimationFrame(frame); clearTimeout(panTimer); refresh.current = () => {}; recenter.current = () => {}; cy.current?.destroy(); cy.current = null; };
  }, [left, right, common]);
  ...
  return <div className="comparison-map">
    ...
    {failed ? <p role="status">La carte n’est pas disponible dans ce navigateur. Les cartes et leurs sources restent accessibles ci-dessous.</p> : <>
      ...
      <div className="comparison-map-tools"><button className="icon-button" aria-label="Zoom avant de la comparaison" onClick={() => zoomBy(1.25)}><Plus size={17} /></button><button className="icon-button" aria-label="Zoom arrière de la comparaison" onClick={() => zoomBy(1 / 1.25)}><Minus size={17} /></button><button className="icon-button" aria-label="Recentrer la comparaison" onClick={() => recenter.current()}><Maximize size={16} /></button><span>Un trait regroupe les déclarations d’un seul parcours. Touchez une entité pour ses preuves.</span></div>
    </>}
