...
export function Comparison({ data, left, right, categories, selected, presentation = 'map', mode = 'common', guided = false, onMode, onSelect, onPresentation, onLeft, onRight, onExplore, onClose }: {
  ...
  return <section className="comparison-view" aria-label="Comparer deux personnes">
    <div className="comparison-heading"><div>{!guided && <><span className="eyebrow">Comparer deux parcours</span><h2>Qu’ont-ils en commun ?</h2></>}</div><button className="icon-button" aria-label="Fermer la comparaison" onClick={onClose}><X size={19} /></button></div>
    {!guided && <p className="comparison-lede">Ont-ils fréquenté les mêmes écoles, travaillé dans les mêmes institutions ou rejoint les mêmes partis ? Choisissez deux personnes pour comparer.</p>}
    <div className="compare-pickers">
      ...
      <div>{right ? <div className="compare-person"><EntityAvatar entity={right} className="mini-avatar ochre" /><strong>{right.label}</strong></div> : <p className="compare-person muted">Choisir une deuxième personne</p>}<EntitySearch data={data} peopleOnly exclude={left.id} onSelect={onRight} label="Deuxième personne à comparer" placeholder="Rechercher une personne…" />{right?.image && <ImageCredit image={right.image} />}</div>
    </div>
    ...
    {right && mode === 'common' && <><div className="comparison-result-heading"><strong>{common.length} {common.length === 1 ? 'point commun trouvé' : 'points communs trouvés'}</strong><span>Selon les filtres actifs</span></div>
      <p className="comparison-counts">{institutions} institutions · {common.length - institutions} autres éléments du parcours · {statementCount} mentions dans les sources</p>
      ...
      {common.length > 0 && <><div className="view-toggle comparison-display" role="group" aria-label="Affichage de la comparaison"><button aria-pressed={presentation === 'map'} onClick={() => onPresentation('map')}>Carte comparative</button><button aria-pressed={presentation === 'cards'} onClick={() => onPresentation('cards')}>Détails et sources</button></div>{presentation === 'map' && <ComparisonGraph left={left} right={right} common={common} selected={selected} onSelect={inspect} />}</>}
      {common.map(connection => <article className={`common-card ${selected === connection.entity.id ? 'is-selected' : ''}`} id={`common-${connection.entity.id}`} key={connection.entity.id}>
        ...
        {!supportsPeriods(connection.entity) && <p className="section-caption">Un même intitulé de fonction ne veut pas forcément dire une même institution.</p>}
      ...
      {!common.length && <div className="empty-state"><GitCompareArrows size={28} /><strong>Aucun point commun dans cette vue</strong><p>Essayez d’autres filtres ou une autre personne. Les parcours renseignés restent incomplets : un point commun peut nous manquer.</p></div>}
    </>}
  </section>;
}
...
function ComparisonPeriods({ connection }: { connection: Parameters<typeof comparisonPeriods>[0] }) {
  ...
  return <div className="comparison-periods"><strong>{periods.total} comparaisons de passages</strong><p>{[
    periods.documented ? `${periods.documented} avec une période commune établie` : '',
    periods.possible ? `${periods.possible} avec des dates qui pourraient se recouper` : '',
    periods.outside ? `${periods.outside} à des périodes différentes` : '',
    periods.unknown ? `${periods.unknown} avec des dates trop imprécises` : '',
  ].filter(Boolean).join(' · ')}</p><small>Les dates sont comparées deux à deux. Cela ne prouve ni une rencontre, ni la présence de tout le groupe au même moment.</small></div>;
}
