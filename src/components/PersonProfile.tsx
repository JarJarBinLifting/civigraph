...
export function PersonProfile({ data, person, mode, onMode, onSelect, onExplore, onEvidence, onConnections }: { data: GraphData; person: Entity; mode: 'overview' | 'career'; onMode: (mode: 'overview' | 'career') => void; onSelect: (id: string) => void; onExplore: (id: string) => void; onEvidence: (id: string) => void; onConnections: () => void }) {
  ...
  return <div className="person-biography">
    ...
    {mode === 'career' ? <section className="career-timeline" aria-label="Parcours chronologique">
      <h3>Les étapes du parcours</h3>
      <p className="profile-scope">Les études, les emplois et les fonctions sont présentés dans l’ordre des dates connues. Certaines activités ont pu avoir lieu en même temps.</p>
      ...
      {!career.dated.length && <p className="section-caption">Nous n’avons pas de dates assez précises pour organiser ce parcours.</p>}
      {career.undated.length > 0 && <><h3>Dates non précisées</h3><ol className="undated-career">{career.undated.map(entry => <CareerItem key={entry.key} entry={entry} onEvidence={onEvidence} onSelect={onSelect} onExplore={onExplore} />)}</ol></>}
    </section> : <>
    ...
    <p className="profile-scope">Ces informations concernent l’ensemble du parcours. Si la date de fin manque, on ne peut pas en déduire que le poste est toujours occupé.</p>
    {milieuProfile && milieuProfile.milieus.length >= 2 && <section className="profile-section"><h3>Les secteurs de son parcours</h3><MilieuEvidence profile={milieuProfile} data={data} /></section>}
    {profile.sections.map(section => <section className="profile-section" key={section.category}>
      ...
      {section.total > section.facts.length && <small className="profile-more">{section.total - section.facts.length} autres éléments dans Connexions</small>}
    </section>)}
    <button className="subtle-link profile-all" onClick={onConnections}>Voir tous ses liens<ArrowUpRight size={13} /></button>
    </>}
  </div>;
}
