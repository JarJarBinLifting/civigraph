...
export function Explorer({ data, initialView, initialDetailOpen = false }: { data: GraphData; initialView: ViewState; initialDetailOpen?: boolean }) {
  ...
  const [showDetail, setShowDetail] = useState(initialDetailOpen && !['circles', 'milieus', 'guided'].includes(initialView.systemLens ?? '') && initialView.spotlight !== 'off' && (!initialView.preset || initialView.selected !== initialView.root || Boolean(initialView.edge)));
  ...
  useEffect(() => {
    const restore = () => {
      ...
      setShowDetail(selection && !['circles', 'milieus', 'guided'].includes(next.systemLens ?? '') && (!next.preset || next.selected !== next.root || Boolean(next.edge)));
  ...
  function restoreExploration(next: ViewState, adjusted: boolean) {
    ...
    window.history.pushState(null, '', `${window.location.pathname}${serializeView(next)}`);
    setComparisonOpen(Boolean(next.compare)); setShowDetail(!['circles', 'milieus', 'guided'].includes(next.systemLens ?? '') && next.spotlight !== 'off' && (!next.preset || next.selected !== next.root || Boolean(next.edge))); setShowFilters(false); setModal(null);
    if (next.preset) setPresetRevision(revision => revision + 1);
  ...
  return <div className="app-shell atlas-explorer map-first">
    ...
    <aside className="desktop-notice" aria-label="Conseil d’utilisation">
      ...
      <p><strong>Plus confortable sur ordinateur.</strong> Vous aurez plus de place pour lire la carte et comparer les parcours.</p>
    </aside>
    <main id="exploration" ref={workspace} role={enlarged ? 'dialog' : undefined} aria-modal={enlarged || undefined} aria-label={enlarged ? 'Carte agrandie' : undefined} className={`workspace ${comparisonOpen ? 'is-comparing' : ''} ${!showDetail ? 'detail-closed' : ''} ${enlarged ? 'map-expanded' : ''}`}>
      ...
      <aside id="exploration-filters" className={`sidebar ${showFilters ? 'mobile-open' : ''}`} aria-label="Filtres et parcours">
        ...
        <div className="scope-section"><div className="section-heading"><h2>Périmètre</h2><Info size={13} /></div><p><CalendarIcon />{temporal === 'same' && periodContext.anchor ? periodContext.anchor.cohort?.label ?? periodLabel(periodContext.anchor) : 'Toutes les périodes'}</p><span>{temporal === 'same' ? 'Les parcours dont les dates sont incomplètes restent consultables dans les fiches.' : 'Les dates sont précisées dans les fiches.'}</span></div>
        ...
        <div className="suggested-paths"><span className="eyebrow">Une piste à explorer</span><button onClick={() => startFrom(entitiesById.get('Q273579')!)}><span className="path-icon"><GraduationCap size={18} /></span><span><strong>Les parcours de l’ENA</strong><small>Retrouver les anciens élèves</small></span><ArrowUpRight size={15} /></button><button onClick={() => startFrom(entitiesById.get('Q1587677')!)}><span className="path-icon blue"><Landmark size={17} /></span><span><strong>Passages à Matignon</strong><small>Les Premiers ministres et leurs parcours</small></span><ArrowUpRight size={15} /></button></div>
        <div className="sidebar-bottom"><div className="prototype-tag"><span className="status-dot" />Version en cours de développement</div><p>{data.meta.peopleCount} personnes recensées</p><button onClick={() => setModal('corpus')}>Voir les personnes recensées<ArrowUpRight size={12} /></button></div>
      </aside>
      ...
      <section className={`exploration-center${periodContext.institution && !comparisonOpen ? ' has-period-controls' : ''}`} aria-label="Vue d’exploration">
        ...
        {comparisonOpen && view.journey && <button className="return-global-map" onClick={() => { update({ graphView: 'system', systemLens: 'guided', journeyStep: 2, compare: null, preset: undefined }); setComparisonOpen(false); setShowDetail(false); }}>← Revenir à l’exploration guidée</button>}
        {comparisonOpen ? <Comparison guided={Boolean(view.preset)} data={data} left={defaultComparisonLeft} right={compare ? entitiesById.get(compare) : undefined} categories={categories}
          ...
          <div className="graph-topbar">
            <div className="graph-heading">{!view.preset && <div className="breadcrumb"><span>Parcours et relations</span><h1 aria-label={isSystem ? "Vue du système" : `Réseau : ${shortLabel(focusEntity)}`}>{isSystem ? "La carte des réseaux politiques" : shortLabel(focusEntity)}</h1></div>}
              {isSystem && !view.preset && <p className="atlas-description">Écoles, partis, institutions, entreprises : explorez les parcours et les liens entre responsables politiques.</p>}
              <div className="graph-meta"><span>{visible.entities.length} fiches</span><span>{visible.relations.length} mentions</span>{isSystem && <span>{system.connections.length} liens</span>}{!showDetail && hasSelection && <button className="subtle-link" onClick={() => { setShowDetail(true); setHasSelection(true); }}>Ouvrir la fiche<ArrowUpRight size={14} /></button>}</div>
            </div>
            <div className="map-display-controls">
              <MapPopover label="Affichage" icon={<PanelsTopLeft size={17} />} status={view.mode === 'list' ? 'Liste' : isSystem ? `Vue d’ensemble · ${view.reading === 'individuals' ? 'Individus' : 'Groupes'}` : 'Centrée'}>
                <p className="map-control-label">Présentation</p><div className="view-toggle" role="group" aria-label="Mode d’affichage"><button aria-pressed={view.mode === 'graph'} onClick={() => update({ mode: 'graph' })}><Network size={16} />Carte</button><button aria-pressed={view.mode === 'list'} onClick={() => update({ mode: 'list' })}><List size={16} />Liste</button></div>
                <p className="map-control-label">Perspective</p><div className="view-toggle" role="group" aria-label="Perspective du graphe"><button aria-pressed={isSystem} onClick={() => changeGraphView('system')}><Network size={16} />Vue d’ensemble</button><button aria-pressed={!isSystem} onClick={() => changeGraphView('centered')}><GitBranch size={16} />Centrée</button></div>
                ...
                <p className="map-control-note">{isSystem ? 'Toutes les personnes et institutions recensées sur une même carte.' : 'Les liens autour de la personne ou de l’institution choisie.'}</p>
              </MapPopover>
              <MapPopover label="Explorer" icon={<Compass size={17} />} status={isSystem && view.systemLens === 'guided' ? 'Explorations guidées' : isSystem && view.systemLens === 'circles' ? 'Cercles entre partis' : isSystem && view.systemLens === 'milieus' ? 'Parcours entre milieux' : isSystem && view.systemLens === 'common' ? 'Points communs' : isSystem && view.systemLens === 'institutions' ? 'Institutions' : undefined} className="map-explore-tools">
                <ExplorationEntries onChoose={setPresetPicker} />
                <div className="map-explore-links"><button aria-pressed={isSystem && view.systemLens === 'guided'} onClick={() => { update({ graphView: 'system', mode: 'graph', systemLens: 'guided', compare: null }); setShowDetail(false); setComparisonOpen(false); }}>Équipes et promotions · visite guidée<ArrowUpRight size={15} /></button><button aria-pressed={isSystem && view.systemLens === 'circles'} onClick={() => { update({ graphView: 'system', mode: 'graph', systemLens: 'circles', compare: null }); setShowDetail(false); setComparisonOpen(false); }}>Cercles entre partis<ArrowUpRight size={15} /></button><button aria-pressed={isSystem && view.systemLens === 'milieus'} onClick={() => { update({ graphView: 'system', mode: 'graph', systemLens: 'milieus', compare: null }); setShowDetail(false); setComparisonOpen(false); }}>Parcours entre milieux<ArrowUpRight size={15} /></button><button aria-pressed={isSystem && view.systemLens === 'common'} onClick={() => { update({ graphView: 'system', mode: 'graph', systemLens: 'common' }); }}>Points communs{view.group?.length ? ` (${view.group.length})` : ''}<ArrowUpRight size={15} /></button><button aria-pressed={isSystem && view.systemLens === 'institutions'} onClick={() => update({ graphView: 'system', mode: 'graph', systemLens: 'institutions' })}>Index des institutions<ArrowUpRight size={15} /></button></div>
                ...
                <a className="map-help-link" href="/methode"><BookOpen size={15} />Sources et méthode<ArrowUpRight size={13} /></a>
          ...
          <div className="graph-bottom"><span><Info size={13} />Un lien sur la carte ne signifie pas que ces personnes se connaissent.</span><button onClick={() => setModal('method')}>Lire la méthode<ArrowUpRight size={12} /></button></div>
        </>}
      </section>
    ...
    <footer className="app-footer"><span><BookOpen size={12} />Wikidata et sources officielles · {importedDate}</span><a href="/methode">Sources et méthode</a><button onClick={() => setModal('method')}>À propos de Civigraph<ArrowUpRight size={12} /></button></footer>
    ...
    {modal === 'share' && <Modal title="Partager cette exploration" onClose={() => setModal(null)}><div className="modal-emblem"><Share2 size={25} /></div><p>Ce lien conserve votre sélection, vos filtres et votre comparaison. Envoyez-le pour partager cette vue.</p><label className="share-label" htmlFor="share-url">Lien vers cette vue</label><div className="share-input"><input id="share-url" readOnly value={shareUrl} onFocus={event => event.target.select()} /><button className="primary-button" onClick={copyShare}>{copied ? <Check size={16} /> : <Copy size={16} />}{copied ? 'Copié' : 'Copier'}</button></div>{copyError && <p className="source-limit" role="status">La copie automatique est indisponible. Sélectionnez le lien puis utilisez Ctrl+C ou Cmd+C.</p>}<p className="local-share-note">Le lien reprend la vue affichée. Les explorations enregistrées dans votre navigateur ne sont pas partagées.</p></Modal>}
    {modal === 'method' && <Modal title="Comprendre les liens" onClose={() => setModal(null)}><p className="modal-lede">D’où viennent les informations ?</p><p>Civigraph rassemble des informations sur les études, les emplois, les fonctions et les appartenances politiques. Elles viennent de Wikidata, des assemblées parlementaires, de déclarations HATVP et de listes publiées par les institutions. Cliquez sur un lien pour retrouver sa source.</p><div className="method-grid"><article><span>01</span><h3>Retrouver la source</h3><p>Chaque fiche renvoie à la page Wikidata utilisée ou au document officiel, avec sa page ou son article lorsqu’ils sont connus. Les dates et les fonctions sont celles de la source.</p></article><article><span>02</span><h3>Vérifier les dates</h3><p>Le filtre « Même période » garde les passages dont les dates se recoupent, ou les membres d’une même liste officielle datée. Les autres restent accessibles dans les fiches. Une date de fin absente ne veut pas dire que la personne occupe encore le poste.</p></article><article><span>03</span><h3>Ce qu’un lien ne dit pas</h3><p>Deux personnes peuvent avoir fréquenté la même école sans s’être rencontrées. Une même promotion, une nomination et un mandat apportent des informations différentes. Une présence à une date précise ne renseigne pas toute une période.</p></article><article><span>04</span><h3>Une sélection encore partielle</h3><p>Le site recense {data.meta.peopleCount} personnes. Ce n’est ni un annuaire complet ni un échantillon représentatif. Les informations ne sont pas mises à jour en continu.</p></article></div><div className="method-note"><Info size={19} /><p>Les informations de Wikidata ne sont pas toutes vérifiées une à une. {wikidataRelations.filter(relation => relation.references.some(reference => reference.urls.length)).length} sur {wikidataRelations.length} renvoient à une référence extérieure. Les autres sont signalées comme étant à recouper. {officialCount} mentions supplémentaires viennent de documents publics identifiés. Les données HATVP décrivent ce qui a été déclaré à leur date de dépôt.</p></div><a className="subtle-link" href="https://www.wikidata.org/wiki/Wikidata:Data_access/fr" target="_blank" rel="noopener noreferrer">Accès aux données et licence Wikidata<ArrowUpRight size={14} /></a><p className="section-caption">Autres sources : <a href="https://data.assemblee-nationale.fr/acteurs/historique-des-deputes" target="_blank" rel="noopener noreferrer">Assemblée nationale</a> · <a href="https://www.integritywatch.fr/" target="_blank" rel="noopener noreferrer">Integrity Watch France</a> · <a href="https://www.hatvp.fr/open-data/" target="_blank" rel="noopener noreferrer">HATVP</a>.</p></Modal>}
    {modal === 'corpus' && <Modal title="Les personnes recensées" onClose={() => setModal(null)}><p>La sélection rassemble des responsables politiques français et des personnes ayant fréquenté les mêmes écoles, entreprises ou organisations. Une partie des profils vient de recherches dans Wikidata, limitées à 60 personnes par institution. Des responsables nationaux et des parlementaires ont ensuite été ajoutés. La sélection reste partielle et ne représente pas toute la vie politique.</p><div className="corpus-stats"><div><strong>{data.meta.peopleCount}</strong><span>personnalités</span></div><div><strong>{data.meta.entityCount}</strong><span>fiches</span></div><div><strong>{data.meta.relationCount}</strong><span>mentions dans les sources</span></div></div><p className="section-caption">Import Wikidata du {importedDate} · données Wikidata sous CC0. Les documents officiels conservent leurs conditions de réutilisation.</p><input className="corpus-search" aria-label="Filtrer les personnes recensées" value={corpusQuery} onChange={event => setCorpusQuery(event.target.value)} placeholder="Rechercher un nom…" /><div className="corpus-list">{filteredPeople.map(entity => <button key={entity.id} onClick={() => startFrom(entity)}><EntityAvatar entity={entity} /><span>{entity.label}</span><ArrowUpRight size={15} /></button>)}</div>{!filteredPeople.length && <p className="empty-search">Aucune personne trouvée. Essayez un autre nom.</p>}</Modal>}
    <span className="sr-only" aria-live="polite">{isSystem ? 'Vue d’ensemble.' : `Vue centrée sur ${shortLabel(focusEntity)}.`} {visible.entities.length} fiches dans le réseau. {view.mode === 'graph' ? `${visible.entities.length} fiches et ${visible.relations.length} mentions affichées.` : `${visible.relations.length} mentions dans la liste.`} {temporal === 'same' ? 'Filtre par période actif.' : 'Toutes les périodes.'}</span>
  </div>;
}
