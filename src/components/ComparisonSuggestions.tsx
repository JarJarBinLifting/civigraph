...
export function ComparisonSuggestions({ data, person, categories, onChoose }: {
  ...
  return <section className="comparison-discovery" aria-label="Suggestions de comparaison">
    <h3>Avec qui comparer ce parcours ?</h3>
    ...
    {suggestions.length ? <>
      <div className="comparison-candidates">{suggestions.slice(0, 6).map(({ person: candidate, connections, sharedPeriodCount }) => <article className="comparison-candidate" key={candidate.id}>
        ...
        <p className="candidate-periods">{sharedPeriodCount ? `${sharedPeriodCount} avec des dates qui se recoupent` : 'Pas de période commune établie'}</p>
        <ul>{connections.slice(0, 3).map(connection => <li key={connection.entity.id}><span>{shortLabel(connection.entity)}</span>{connection.sharedPeriod && <small>Dates communes établies</small>}</li>)}</ul>
      ...
      <p className="discovery-method">{Math.min(suggestions.length, 6)} profils proposés sur {suggestions.length}. Classés par nombre d’institutions distinctes en commun, puis de périodes communes établies. Plusieurs sources pour une même institution ne la font compter qu’une fois.</p>
    ...
    <p className="discovery-limit">Ces personnes ont des institutions en commun dans les parcours recensés. Cela ne signifie pas qu’elles se connaissent. Comparez-les pour voir les dates et les sources.</p>
  </section>;
}
