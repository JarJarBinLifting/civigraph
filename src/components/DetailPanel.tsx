...
export function RelationEvidence({ relation, data, compact = false }: { relation: Relation; data: GraphData; compact?: boolean }) {
  ...
  return <article className={`evidence-card ${compact ? 'compact' : ''}`}>
    ...
    {relation.contexts?.length ? <p className="context-note">La source précise le contexte : {relation.contexts.map(context => context.label).join(' · ')}.</p> : null}
    <div className="evidence-links">
      <a href={relation.statementUrl} target="_blank" rel="noopener noreferrer"><BookOpen size={14} />{relation.evidence?.kind === 'declaration' ? 'Consulter la déclaration HATVP' : relation.evidence ? 'Consulter le document officiel' : 'Voir l’information sur Wikidata'}<ArrowUpRight size={14} /></a>
      {relation.revisionUrl && <a href={relation.revisionUrl} target="_blank" rel="noopener noreferrer">Version utilisée par Civigraph<ArrowUpRight size={13} /></a>}
    ...
    {!urls.length && <p className="source-limit">{statedIn.length ? 'Wikidata cite une publication, mais ne donne pas de lien pour la consulter.' : 'Wikidata ne donne pas de lien vers une source extérieure.'} Cette information reste à recouper.</p>}
  </article>;
}
...
interface Props {
  ...
  systemProfile?: ReactNode;
  systemConnections?: ReactNode;
}

export function DetailPanel({ data, entity, categories, temporal, periodAnchor, selectedEdge, focused, onSelect, onEdge, onExpand, onClose, onAllPeriods, onCareer, systemProfile, systemConnections }: Props) {
  ...
  return <aside className="detail-panel" aria-label={`Fiche de ${entity.label}`}>
    <div className="panel-topline"><span className="eyebrow">Fiche</span><button className="icon-button" aria-label="Fermer la fiche" onClick={onClose}><X size={17} /></button></div>
    <div className="entity-profile">
      ...
      <a className="subtle-link" href={entityPath(entity.id)}>Voir la fiche complète<ArrowUpRight size={13} /></a>
      {(entity.wikidataUrl || entity.sourceUrl) && <a className="subtle-link" href={entity.wikidataUrl ?? entity.sourceUrl} target="_blank" rel="noopener noreferrer">{entity.wikidataUrl ? 'Fiche Wikidata' : entity.sourceLabel ?? 'Source de la fiche'}<ArrowUpRight size={13} /></a>}
    ...
    <div className="profile-stats"><div><strong>{connectionCount}</strong><span>fiches liées</span></div><div><strong>{allRelations.length}</strong><span>mentions</span></div></div>
    <button className="primary-button expand-button" onClick={() => onExpand(entity.id)} disabled={focused}><GitBranch size={16} />{focused ? 'Au centre de la carte' : 'Explorer autour de cette fiche'}{!focused && <Plus size={15} />}</button>
    ...
    <div className="panel-content" role="tabpanel" aria-label={activeTab === 'profile' ? 'Profil de la personne' : activeTab === 'connections' ? 'Connexions de l’entité' : 'Sources des relations'}>
      {activeTab === 'profile' ? <>{systemProfile}<PersonProfile data={data} person={entity} mode={profileMode} onMode={setProfileMode} onSelect={onSelect} onExplore={onExpand} onEvidence={onEdge} onConnections={() => { setTab('connections'); onEdge(null); }} /></> : activeTab === 'connections' ? systemConnections ?? <>
        <p className="section-caption">{temporal === 'same' ? 'Participations selon la période retenue' : categories.length === 5 ? 'Tous les liens recensés' : 'Liens selon les filtres actifs'}{entity.id === 'Q2986712' && ' · sélection non exhaustive'}</p>
        {[...groups.entries()].map(([key, group]) => {
          ...
          return <div className="connection-row" key={key}>
            ...
            <div><span className="connection-category">{categoryInfo[relation.category].singular}</span>
              ...
              <small>{group.length > 1 ? `${group.length} mentions · plusieurs périodes` : relation.cohort?.label ?? periodLabel(relation)}</small>
            </div>
        ...
        <div className="source-intro"><BookOpen size={16} /><p>Voici les informations qui expliquent les liens de cette fiche. Ouvrez les références pour lire les documents d’origine.</p></div>
        {selectedEdge ? <><button className="subtle-link" onClick={() => { onEdge(null); setTab('sources'); }}>Voir toutes les informations ({allRelations.length})</button><RelationEvidence relation={selectedEdge} data={data} /></> : allRelations.map(relation => <RelationEvidence key={relation.id} relation={relation} data={data} compact />)}
      </>}
      {activeTab !== 'profile' && !allRelations.length && !selectedEdge && <div className="empty-state"><GitBranch size={25} /><strong>Aucun lien avec ces filtres</strong><p>{temporal === 'same' ? 'Les dates disponibles ne permettent pas de retenir un lien sur cette période avec ces catégories.' : 'Réactivez une catégorie pour retrouver les liens de cette fiche.'}</p></div>}
    ...
    <div className="panel-footer"><span className="status-dot" />{allRelations.some(relation => relation.evidence) ? 'Documents officiels et Wikidata' : 'Wikidata'} · {allRelations.filter(hasExternalReference).length} mentions avec une référence consultable</div>
  </aside>;
}
