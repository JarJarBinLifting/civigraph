...
export async function generateMetadata({ params }: Props) {
  ...
  if (!entity) notFound();
  const description = `${entity.label}${entity.description ? `, ${entity.description}` : ''}. Retrouvez les parcours, les dates et les sources rassemblés sur Civigraph.`;
...
export default async function EntityDocument({ params }: Props) {
  ...
  const careerSections = career ? [{ heading: 'Passages datés', entries: career.dated }, { heading: 'Dates non précisées', entries: career.undated }] : [];
  return <DocumentLayout>
    ...
    <div className="document-title"><p className="eyebrow">Fiche · {typeInfo[entity.type].label}</p><h1>{entity.label}</h1><p>{entity.description}</p><a className="primary-button" href={`/?root=${encodeURIComponent(id)}&time=all`}>Explorer sur la carte</a></div>
    <div className="document-statistics"><span><strong>{neighbors.size}</strong> fiches liées</span><span><strong>{relations.length}</strong> mentions dans les sources</span><span>Données au {new Date(data.meta.supplementedAt ?? data.meta.fetchedAt).toLocaleDateString('fr-FR', { timeZone: 'UTC' })}</span></div>
    <p className="document-limit">Cette fiche reprend les informations disponibles sur le site. Les données Wikidata ne sont pas toutes vérifiées une à une. Plusieurs mentions peuvent décrire le même fait : leur nombre ne dit pas combien de personnes se connaissent.</p>
    {(entity.wikidataUrl || entity.sourceUrl) && <a className="document-source" href={entity.wikidataUrl ?? entity.sourceUrl} target="_blank" rel="noopener noreferrer">{entity.wikidataUrl ? 'Voir la fiche Wikidata' : entity.sourceLabel ?? 'Source de cette fiche'} ↗</a>}
    ...
    {profile && <section className="document-overview"><h2>En quelques lignes</h2>{profile.paragraphs.map(paragraph => <p key={paragraph}>{paragraph}</p>)}<p>Cette fiche reprend le parcours sans les filtres de la carte. Une date de fin absente ne veut pas dire que la personne occupe encore le poste.</p></section>}
    {career && <section className="document-career"><h2>Parcours chronologique</h2><p>Plusieurs activités peuvent avoir lieu en même temps. Les dates manquantes restent inconnues.</p>{careerSections.map(({ heading, entries }) => entries.length > 0 && <div key={heading}><h3>{heading}</h3><ol>{entries.map(entry => <li key={entry.key}><span>{periodLabel(entry.relations[0])}</span><div><small>{categoryInfo[entry.category].singular}</small><a href={entityPath(entry.entity.id)}>{entry.entity.label}</a>{entry.relations[0].role && <p>{entry.relations[0].role}</p>}{entry.relations.map((relation, i) => <a key={relation.id} className="document-source" href={`#relation-${encodeURIComponent(relation.id)}`}>Consulter la source {entry.relations.length > 1 ? i + 1 : ''} de ce passage</a>)}</div></li>)}</ol></div>)}</section>}
    <section className="document-relations"><h2>Relations et sources</h2><p>Ouvrez une ligne pour retrouver les dates et les documents qui la justifient.</p>{relations.map(relation => {
      ...
      return <details className="document-relation" key={relation.id} id={`relation-${encodeURIComponent(relation.id)}`}>
        ...
        <div className="document-links"><a href={entityPath(source.id)}>{source.label} : fiche</a><a href={entityPath(target.id)}>{target.label} : fiche</a><a href={relation.statementUrl} target="_blank" rel="noopener noreferrer">{relation.evidence ? 'Consulter le document source' : 'Voir l’information sur Wikidata'} ↗</a>{relation.revisionUrl && <a href={relation.revisionUrl} target="_blank" rel="noopener noreferrer">Version utilisée par Civigraph ↗</a>}{references.filter(url => url !== relation.statementUrl.split('#')[0]).map((url, i) => <a key={url} href={url} target="_blank" rel="noopener noreferrer">Référence {i + 1} · {new URL(url).hostname} ↗</a>)}</div>
    ...
    })}{!relations.length && <p>Aucun lien n’a encore été ajouté à cette fiche.</p>}</section>
  </DocumentLayout>;
}
