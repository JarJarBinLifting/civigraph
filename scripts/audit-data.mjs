...
const after = [...new Map([...current.relations, ...currentOfficial.relations, ...network.relations, ...assembly.relations, ...integrity.relations, ...rosterRelations(institutionRosters), ...before].map(relation => [relation.id, relation])).values()];
const entities = new Map();
for (const data of [original, attali, network, assembly, integrity, current, currentOfficial]) for (const entity of data.entities) entities.set(entity.id, { ...entity, inCorpus: entity.inCorpus || Boolean(entities.get(entity.id)?.inCorpus) });
