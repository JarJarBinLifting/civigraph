...
Le menu **Explorer → Équipes et promotions · visite guidée** ouvre huit groupes sourcés : deux cabinets ministériels et six promotions/programmes. Chaque visite relie composition, passages ultérieurs et comparaison **Se sont-ils croisés ?**, avec partage et sauvegarde de l’étape. Le complément de cabinets porte le corpus à **553 personnes, 2 589 entités et 8 055 déclarations**. [Couverture et règles des visites](docs/guided-networks.md).
...
Le cadrage initial inclut tous les nœuds et les repères temporels, en équilibrant leur étendue réelle, même pour les petits réseaux asymétriques. La personne explorée reste l’origine des couronnes sans être nécessairement au milieu de l’écran. Pour les réseaux denses, les noms se révèlent au zoom, au survol ou à la sélection ; le nœud actif reste identifiable. La distance décrit un écart entre périodes, sans démontrer une rencontre ou une proximité personnelle.

La carte utilise des disques à monogramme pour les personnes et des pictogrammes blancs sur des symboles pleins, colorés par type d’entité. Les liens reprennent des couleurs contrastées selon leur catégorie. La comparaison affiche les portraits sourcés des deux personnes lorsqu’ils sont disponibles, avec les initiales en secours. Ses liens indiquent les périodes de participation ; plusieurs passages restent séparés et les dates absentes sont signalées. Sur ordinateur, les réseaux de 28 entités au maximum bénéficient de symboles agrandis, d’un espacement plus généreux et de noms de 16 à 20 pixels CSS. Les vues denses et mobiles gardent des noms de 14 à 15 pixels, sur deux lignes au maximum. Leur placement privilégie le centre, la sélection et le parcours ; masquer une étiquette ne retire jamais un nœud. Ouvrir une fiche conserve le zoom et la zone observée. « Lire les distances » précise la légende ; la taille du centre indique uniquement son rôle dans l’exploration.
...
Les listes de passages et de sources suivent le même ordre chronologique, du plus ancien au plus récent : fiches Connexions/Sources, repères affichés du profil, comparaison, segments des chemins, mode Liste et notices documentaires. Le tri utilise le début connu, sinon la date d’attestation, sinon la fin connue. Les périodes sans date exploitable viennent à la fin ; aucune borne manquante n’est complétée. Les regroupements par personne ou institution restent conservés.
...
npm run data:import:current
npm run data:audit:current
npm run data:import:assembly
npm run data:import:integrity
npx tsx scripts/audit-data.mjs
npm run data:layout
npm test
npm run build
```
...
L’indexation est **désactivée par défaut**. `.env.example` documente `CIVIGRAPH_INDEXING=false` et `CIVIGRAPH_SITE_URL` vide. La production Cloudflare autorisée utilise `https://civigraph.org`, avec `CIVIGRAPH_INDEXING=true` dans `wrangler.jsonc`. Le développement et les environnements de prévisualisation identifiés restent en `noindex` ; laisser le drapeau désactivé sur toute autre prévisualisation. Voir [le déploiement Cloudflare](docs/cloudflare.md).

Après activation, l’accueil sans paramètres, la méthode et les notices éligibles émettent une URL canonique, les métadonnées de partage et `index, follow`. L’accueil décrit aussi le site avec des données structurées `WebSite`. Une notice éligible possède un nom, une source et au moins une relation ; les personnes appartiennent au corpus et les institutions ont un contexte explicite. Les intitulés de fonctions génériques restent exclus du sitemap. Les vues avec paramètres, filtres et comparaisons restent en `noindex, follow`. En production, `robots.txt` autorise le crawl pour que les moteurs puissent lire ces consignes et charger les ressources de rendu. Sans activation valide, le sitemap est vide et `robots.txt` interdit l’exploration.
